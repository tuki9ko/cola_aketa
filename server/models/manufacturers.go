...
var ManufacturerWhere = struct {
	...
	ID:   whereHelperint{field: "\"manufacturers\".\"id\""},
	Name: whereHelperstring{field: "\"manufacturers\".\"name\""},
}
...
func (r *manufacturerR) GetColaTypes() ColaTypeSlice {
	if r == nil {
		return nil
	}
	return r.ColaTypes
...
func (o *Manufacturer) ColaTypes(mods ...qm.QueryMod) colaTypeQuery {
	...
	queryMods = append(queryMods,
		qm.Where("\"cola_types\".\"manufacturer_id\"=?", o.ID),
	)
...
func (manufacturerL) LoadColaTypes(ctx context.Context, e boil.ContextExecutor, singular bool, maybeManufacturer interface{}, mods queries.Applicator) error {
	...
	query := NewQuery(
		qm.From(`cola_types`),
		qm.WhereIn(`cola_types.manufacturer_id in ?`, args...),
	)
	if mods != nil {
		mods.Apply(query)
	}
...
func (o *Manufacturer) AddColaTypes(ctx context.Context, exec boil.ContextExecutor, insert bool, related ...*ColaType) error {
	var err error
...
func Manufacturers(mods ...qm.QueryMod) manufacturerQuery {
	...
	if len(queries.GetSelect(q)) == 0 {
		queries.SetSelect(q, []string{"\"manufacturers\".*"})
	}
...
func FindManufacturer(ctx context.Context, exec boil.ContextExecutor, iD int, selectCols ...string) (*Manufacturer, error) {
	...
	query := fmt.Sprintf(
		"select %s from \"manufacturers\" where \"id\"=$1", sel,
	)
...
func (o *Manufacturer) Insert(ctx context.Context, exec boil.ContextExecutor, columns boil.Columns) error {
	...
	if !cached {
		...
		if len(wl) != 0 {
			...
			cache.query = "INSERT INTO \"manufacturers\" %sDEFAULT VALUES%s"
		}
...
func (o *Manufacturer) Upsert(ctx context.Context, exec boil.ContextExecutor, updateOnConflict bool, conflictColumns []string, updateColumns, insertColumns boil.Columns) error {
	...
	if !cached {
		...
		cache.query = buildUpsertQueryPostgres(dialect, "\"manufacturers\"", updateOnConflict, ret, update, conflict, insert)
...
func ManufacturerExists(ctx context.Context, exec boil.ContextExecutor, iD int) (bool, error) {
	var exists bool
	sql := "select exists(select 1 from \"manufacturers\" where \"id\"=$1 limit 1)"
